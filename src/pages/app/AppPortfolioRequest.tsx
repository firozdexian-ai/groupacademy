...
const PORTFOLIO_COST = 500; // <--- DEFINED COST
...
export default function AppPortfolioRequest() {
  ...
  const { deductCredits, canAfford } = useCredits(); // <--- USE CREDITS HOOK
  ...
  // Logic to determine if it's free.
  // Note: Depending on RLS, count might be restricted. If strict RLS, assume paid to be safe or use Edge Function for global count.
  ...
  const loadPortfolioCount = async () => {
    try {
      // NOTE: This count might be blocked by RLS if user is anon or standard user.
      // Ideally, use an RPC function: supabase.rpc('get_portfolio_request_count')
      const { count, error } = await supabase.from("portfolio_requests").select("*", { count: "exact", head: true });

      if (!error) {
        setPortfolioCount(count || 0);
      } else {
        // Fallback: if we can't read count due to RLS, assume promotion is over to be safe financially
        console.warn("Could not determine global portfolio count, defaulting to non-free.");
        setPortfolioCount(FREE_PORTFOLIO_LIMIT + 1);
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    // --- PAYMENT CHECK LOGIC ---
    if (!isFreePromotion) {
      if (!canAfford("PORTFOLIO")) {
        toast({
          title: "Insufficient Credits",
          description: `This service requires ${PORTFOLIO_COST} credits.`,
          variant: "destructive",
        });
        return;
      }
    ...
    try {
      // 1. Process Payment first (if not free)
      if (!isFreePromotion) {
        const success = await deductCredits("PORTFOLIO", undefined, "Portfolio Request");
        if (!success) {
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        <Card className="text-center">
          <CardHeader>
            ...
            <CardDescription>Our team will contact you on WhatsApp within 24-48 hours.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-3 justify-center">
              <Button onClick={() => navigate("/app/services")}>Back to Services</Button>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4">
          ...
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{remainingFree} spots remaining</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <span className="font-bold text-lg">{PORTFOLIO_COST} Credits</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground mt-2">Get a professionally designed portfolio website</p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 mb-6">
        {steps.map((s, i) => (
          ...
          >
            <div className={`p-2 rounded-full ${i <= currentStepIndex ? "bg-primary/10" : "bg-muted"}`}>{s.icon}</div>
            {i < steps.length - 1 && (
          ...
          {currentStep === "review" && (
            <div className="space-y-4">
              <div className="bg-muted p-4 rounded-lg space-y-2">
                ...
                <p>
                  <strong>Profession:</strong> {selectedCategory?.name || "Not selected"}
                </p>
                <p>
                  <strong>CV:</strong> {effectiveCvUrl ? "Provided" : "Not provided"}
                </p>
                <p>
                  <strong>Certificates:</strong> {formData.certificates.length} uploaded
                </p>

                <div className="mt-4 pt-4 border-t border-gray-200">
                  <p className="flex justify-between font-semibold">
                    <span>Total Cost:</span>
                    <span>{isFreePromotion ? "FREE (Promotion)" : `${PORTFOLIO_COST} Credits`}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            {currentStep !== "review" ? (
              ...
              <Button className="flex-1" onClick={handleSubmit} disabled={isSubmitting}>
                {isSubmitting ? (
                  ...
                  <>
                    {isFreePromotion ? "Submit Request (Free)" : `Pay ${PORTFOLIO_COST} Credits & Submit`}
