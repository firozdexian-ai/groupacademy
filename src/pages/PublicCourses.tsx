  ...
  const { data: courses = [], isLoading: coursesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("content")
        .select("id, title, slug, description, cover_image_url, thumbnail_url, content_type, duration_hours, current_enrollment, instructor_name, modules_count")
    ...
    <div className="min-h-screen flex flex-col bg-background selection:bg-primary/10">
      ...
      <header className="border-b border-border/40 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" onClick={() => navigate("/auth")} className="rounded-xl font-black uppercase text-[10px] tracking-widest px-6 border-primary/20">Sign In</Button>
          </div>
        </div>
      </header>
      ...
      <section className="relative pt-20 pb-24 overflow-hidden border-b border-border/40 bg-muted/20">
        ...
        <div className="container mx-auto px-6 text-center space-y-8 relative z-10 animate-in fade-in duration-700">
          <Badge variant="outline" className="rounded-full px-4 py-1.5 border-primary/20 bg-primary/5 text-primary font-black uppercase text-[10px] tracking-[0.2em]">
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] max-w-4xl mx-auto">
            Upgrade Your <span className="text-primary">Logic.</span> <br />Accelerate Your Career.
          </h1>

          <p className="text-lg md:text-xl text-muted-foreground font-medium max-w-2xl mx-auto leading-relaxed">
            Professional curriculum across 7 tech academies. From AI engineering to modern business management. Start with 250 welcome credits.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <Button size="lg" onClick={() => navigate("/auth?tab=signup&returnTo=/app/learning")} className="h-14 px-10 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/20">
      ...
      {tracks.length > 0 && (
        <section className="container mx-auto px-6 py-16">
          <header className="flex flex-col items-center mb-12 space-y-2">
            ...
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.3em]">Select your industry vertical</p>
          </header>
      ...
      <section className="container mx-auto px-6 py-16 border-t border-border/40">
        <header className="flex flex-col items-center mb-16 space-y-2">
          ...
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.3em]">Trending Learning Artifacts</p>
        </header>

        {coursesLoading ? (
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {[1, 2, 3, 4, 5, 6].map((i) => <Skeleton key={i} className="h-