...
export function ReactionBar({ reactions, userReaction, onReact, disabled }: ReactionBarProps) {
  ...
  return (
    <div className="space-y-2">
            ...
            {topReactions.map((type) => {
              ...
              return (
                <div
                  key={type}
                  className={cn(
                    "h-4 w-4 rounded-full flex items-center justify-center bg-background ring-1 ring-border",
                    config.color
                  )}
                >
      ...
      <div className="flex items-center gap-1 border-t border-border/50 pt-2">
        {(Object.entries(REACTION_CONFIG) as [ReactionType, typeof REACTION_CONFIG.like][])
          .slice(0, 3) // Show only 3 main reactions
          .map(([type, config]) => {
            const Icon = config.icon;
            ...
            return (
              <Button
                ...
                disabled={disabled}
                className={cn(
                  "flex-1 h-9 text-xs gap-1.5 transition-all",
