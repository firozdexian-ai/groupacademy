...
export function JobCard({
  ...
  // CTO FIX: Currency-Aware Formatter for the Job Feed
  const formatSalary = () => {
        ...
        <CardContent className="p-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Badge variant="secondary" className={cn("text-xs", JOB_TYPE_COLORS[job.job_type])}>
    ...
    >
      <CardContent className="p-4 flex flex-col h-full">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          <Badge variant="secondary" className={cn("text-[10px] h-6 px-2 border-0", JOB_TYPE_COLORS[job.job_type])}>
          ...
          {formatSalary() && (
            <Badge
              variant="outline"
              className="text-[10px] h-6 px-2 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20"
            >
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground flex-1">
          ...
          {job.experience_level && <span className="capitalize">{getExperienceLevelLabel(job.experience_level)}</span>}
        </div>
