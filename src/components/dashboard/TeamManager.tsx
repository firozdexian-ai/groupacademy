    ...
    try {
      const fetchProtocol = async () => {
        ...
        const { data: talents, error: talentsError } = await supabase
          .from("talents")
          .select("user_id, email, full_name, phone, profile_photo_url, current_status, created_at")
          .in("user_id", userIds);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-1000 p-4 md:p-6">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-muted/20 p-8 rounded-[40px] border-2 border-border/40 backdrop-blur-md">
        <div className="space-y-1 text-left">
          <div className="flex items-center gap-3 text-rose-500">
            <ShieldAlert className="h-8 w-8 text-rose-500 fill-rose-500/20" />
            <h2 className="text-4xl font-black uppercase tracking-tighter italic leading-none text-foreground">
              Access Control
            </h2>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 italic">
            System Authority & Security Protocol Center
          </p>
        </div>
        <div className="flex gap-3">
          <Badge
            variant="outline"
            className="h-12 px-6 rounded-xl border-2 font-black italic gap-2 text-rose-500 bg-rose-500/5"
          >
            <Activity className="h-4 w-4" /> {teamMembers.length} ACTIVE AUTHORITIES
          </Badge>
          <Button
            variant="outline"
            size="icon"
            onClick={fetchTeamMembers}
            className="h-12 w-12 rounded-xl border-2 hover:bg-rose-500/10 hover:text-rose-500 transition-colors"
          >
            <RefreshCw className="h-4 w-4" />
          </Button>
          <Button
            onClick={() => setIsDialogOpen(true)}
            className="h-12 px-8 rounded-xl font-black uppercase text-[10px] tracking-widest gap-2 bg-rose-600 hover:bg-rose-700 text-white shadow-lg shadow-rose-500/20"
          >
            ...
            <Table>
              <TableHeader className="bg-muted/10 border-b-2 border-border/20">
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="text-right py-6 pr-8">Protocols</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody className="divide-y divide-border/5">
                {teamMembers.map((member) => (
                  <TableRow key={member.id} className="group hover:bg-rose-500/[0.02] transition-colors">
                    <TableCell className="py-6 pl-8">
                      <div className="flex items-center gap-4">
                        <Avatar className="h-10 w-10 border-2 border-border/20 shadow-sm">
                          <AvatarImage src={member.talent?.profile_photo_url || ""} />
                          <AvatarFallback className="bg-muted/50 text-foreground font-black italic">
                    ...
                    <TableCell>
                      <Badge
                        className={cn(
                          "font-black text-[9px] uppercase italic border-2 px-3 py-1",
                          member.role === "admin"
                            ? "bg-rose-500/10 text-rose-600 border-rose-500/20"
                            : "bg-primary/5 text-primary border-primary/20",
                        )}
                      >
                        <Shield className="h-3 w-3 mr-1 inline-block" /> {member.role.replace("_", " ")}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-[10px] font-black italic text-muted-foreground/40 font-mono">
                    ...
                    <TableCell className="text-right pr-8">
                      <div className="flex justify-end gap-1 opacity-20 group-hover:opacity-100 transition-opacity">
                        ...
                        <Button
                          ...
                          className="hover:bg-amber-500/10 hover:text-amber-600 transition-colors"
                        >
                        ...
                        <Button
                          ...
                          className="hover:bg-rose-500/10 text-rose-500 transition-colors"
                        >
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="max-w-md rounded-[40px] border-4 border-border/40 p-10 bg-background overflow-hidden text-left shadow-2xl">
          <div className="h-2 w-full bg-gradient-to-r from-primary to-blue-500 absolute top-0 left-0" />
          {selectedMember && (
            <div className="space-y-8">
              <div className="flex items-center gap-5 border-b border-border/10 pb-6">
                ...
                <div>
                  ...
                  <Badge className="bg-primary/10 text-primary border-none font-black italic text-[10px] mt-2 uppercase px-3">
              ...
              <div className="pt-4 border-t border-border/10">
                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 italic mb-2">
                  Global Instance ID
                </p>
                <code className="text-[10px] font-mono bg-muted/30 px-3 py-2 rounded-lg block text-muted-foreground">
          ...
          <div className="p-8">
            <DialogHeader className="text-left mb-6">
              <DialogTitle className="text-2xl font-black uppercase italic tracking-tighter flex items-center gap-3">
                <ShieldAlert className="h-6 w-6 text-rose-500" /> Grant Access
              </DialogTitle>
            ...
            <div className="space-y-6 text-left">
              ...
              <div className="space-y-2">
                ...
                <Select value={newRole} onValueChange={(v) => setNewRole(v as AppRole)}>
                  <SelectTrigger className="h-14 rounded-xl border-2 font-black uppercase text-[10px] tracking-widest bg-muted/20">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded-xl border-2">
                    <SelectItem value="talent_exec" className="font-bold text-[10px] uppercase tracking-widest">
                      TALENT_SUCCESS_EXECUTIVE
                    </SelectItem>
                    <SelectItem value="admin" className="font-bold text-[10px] uppercase tracking-widest text-rose-500">
                      FULL_SYSTEM_ADMIN
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <Button
                ...
                className="w-full h-14 rounded-xl font-black uppercase italic tracking-widest text-[10px] gap-2 bg-rose-600 hover:bg-rose-700 text-white shadow-lg"
              >
                {isSubmitting ? <RefreshCw className="animate-spin h-4 w-4" /> : <Zap className="h-4 w-4" />} Authorize
                Deployment
              </Button>
            </div>
          </div>
      ...
      >
        <DialogContent className="max-w-md rounded-[40px] border-4 border-border/40 p-0 bg-background text-left overflow-hidden shadow-2xl">
          <div className="h-2 w-full bg-gradient-to-r from-amber-500 to-orange-500" />
          <div className="p-8">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-2xl font-black uppercase italic tracking-tighter flex items-center gap-3">
                <KeyRound className="text-amber-500 h-6 w-6" /> Security Override
              </DialogTitle>
              <DialogDescription className="text-[10px] font-bold uppercase tracking-widest italic text-muted-foreground/60">
                Reset workforce access key for <span className="text-foreground">{resetMember?.talent?.full_name}</span>
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-6">
              {tempPassword ? (
                <div className="p-6 bg-amber-500/5 border-2 border-amber-500/20 rounded-[24px] space-y-4 animate-in zoom-in-95">
                  <p className="text-[10px] font-black uppercase text-amber-600 italic tracking-widest">
                    Temporal Key Generated
                  </p>
                  <div className="flex gap-2">
                    <code className="flex-1 p-4 bg-background rounded-xl border-2 font-mono text-lg font-black tracking-widest text-center">
                      {tempPassword}
                    </code>
                    <Button
                      variant="outline"
                      ...
                      className="h-[60px] w-[60px] rounded-xl border-2 shrink-0"
                    >
                      <Check className="h-6 w-6 text-green-500" />
                    </Button>
                  </div>
                  <p className="text-[9px] font-bold text-muted-foreground italic text-center uppercase tracking-widest">
                    Share this key securely. Force change required on ingress.
                  </p>
                </div>
              ) : resetLink ? (
                <div className="p-6 bg-primary/5 border-2 border-primary/20 rounded-[24px] space-y-4 animate-in zoom-in-95">
                  <p className="text-[10px] font-black uppercase text-primary italic tracking-widest">
                    External Reset Protocol
                  </p>
                  <div className="flex gap-2">
                    <code className="flex-1 p-4 bg-background rounded-xl border-2 text-[10px] font-mono truncate">
                      {resetLink}
                    </code>
                    <Button
                      variant="outline"
                ...
                <>
                  ...
                  >
                    <div className="flex items-center space-x-3 p-5 border-2 rounded-xl cursor-pointer hover:bg-muted/10 transition-all border-border/40 bg-muted/5">
                      <RadioGroupItem value="email" id="email" />
                      <Label
                        htmlFor="email"
                        className="font-black uppercase italic text-xs cursor-pointer tracking-widest"
                      >
                  ...
                  <Button
                    ...
                    className="w-full h-14 rounded-xl font-black uppercase italic tracking-widest text-[10px] gap-2 shadow-lg"
                  >
                    {isResetting ? <RefreshCw className="animate-spin h-4 w-4" /> : <Zap className="h-4 w-4" />}{" "}
                    Recalibrate Keys
                  </Button>
                </>
              )}
              {(tempPassword || resetLink) && (
                <Button
                  variant="outline"
                  className="w-full h-14 rounded-xl font-black uppercase italic tracking-widest text-[10px] border-2"
...
function StatNode({ icon: Icon, label, value }: any) {
  return (
    <div className="p-5 bg-muted/10 rounded-2xl border-2 border-border/10 space-y-1">
      <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60 italic flex items-center gap-2">
