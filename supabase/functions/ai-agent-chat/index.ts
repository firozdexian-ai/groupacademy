...
// Agent system prompts stored in database, but we have fallbacks here
...
serve(async (req) => {
  ...
  try {
    ...
    if (!agentKey || !messages) {
      return new Response(
        JSON.stringify({ error: "agentKey and messages are required" }),
    ...
    console.log(`AI Agent Chat - Agent: ${agentKey}, Messages: ${messages.length}`);
